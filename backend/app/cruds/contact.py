...
from app.models import contact as model
from app.schemas import contact as schemas

def create_contact(db: Session, contact: schemas.ContactCreate):
    db_contact = model.Contact(**model.dict())
    db.add(db_contact)
    db.commit()
...
def get_contacts(db: Session, name: str = None, company_name: str = None):
    query = db.query(model.Contact).join(model.Company)

    if name:
        query = query.filter(model.model.name.ilike(f"%{name}%"))
    if company_name:
        query = query.filter(model.Company.name.ilike(f"%{company_name}%"))
...
def get_contact(db: Session, contact_id: int):
    return db.query(model.Contact).filter(model.model.id == contact_id).first()
